...
- **Audit timestamps come from one clock, and it is Postgres's.** `defaultNow()` is the database clock at microsecond precision; `$onUpdate(() => new Date())` is the application clock truncated to milliseconds, and mixing them lets `updated_at` land up to a millisecond _before_ `created_at` on a fast insert-then-update. It fails about one run in four and reads as a flaky test rather than as what it is. `$onUpdate(() => sql\`now()\`)`.
