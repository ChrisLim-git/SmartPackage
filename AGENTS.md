...
- **There is no application layer and no use-case layer.** The repository and `UnitOfWork` contracts live in `src/domain/interfaces/`, so a flow that needs persistence is a **domain service** in `src/domain/services/` and still imports nothing outside the domain. Route handlers guard, validate, delegate and map — they hold no behaviour, and their tests assert only the HTTP contract.
