...
- **`eslint-plugin-boundaries` classifies a file by its nearest matching ancestor folder name.** A bare pattern like `components` matches `src/4_presentation/components` as readily as the shadcn folder, and the nested files then inherit the wrong layer's rules _silently_. Ordering and `./` prefixes do not change it — this is why presentation components live in `src/4_presentation/views`.
...
- **`boundaries/elements` patterns are bare directories** (`"src/1_domain"`). `"src/1_domain/**/*"` leaves files sitting directly in the folder unclassified, and every policy then skips them **without reporting anything**. `boundaries/external` and `mode:` are both deprecated in v7 — external packages are restricted with `no-restricted-imports` instead.
