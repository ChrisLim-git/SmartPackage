...
- **Every TanStack Query call lives in `hooks/`.** A component renders; it does not fetch. The cache keys are in `hooks/api.ts` so an invalidation and the query it refreshes cannot drift apart.
