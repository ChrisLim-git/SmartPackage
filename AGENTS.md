...
- **`eslint-plugin-boundaries` classifies a file by its nearest matching ancestor folder name.** A bare pattern like `components` matches `src/presentation/components` as readily as the shadcn folder, and the nested files then inherit the wrong layer's rules _silently_. Ordering and `./` prefixes do not change it — this is why presentation components live in `src/presentation/views`.
