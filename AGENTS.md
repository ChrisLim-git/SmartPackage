...
- **Every TanStack Query call lives in `hooks/`, one hook per file**, named for the hook it exports. A component renders; it does not fetch. `hooks/api.ts` is the exception and is not a hook — it holds `readJson`, `postJson` and the cache keys, so an invalidation and the query it refreshes cannot drift apart.
- **`utils/` at the root is the test doubles**, not a general-purpose folder: in-memory repositories, stub generators, the pool onto `smartpackage_test`. Production code importing any of it is blocked by `no-restricted-imports`, and that guard is **restated in every block that sets the rule** — flat config replaces a rule rather than merging it, so the domain's own package ban silently dropped the guard until a probe found it. Its element pattern also needs `partialMatch: false`, or a bare `utils` also matches `src/domain/utils` and reclassifies the value objects out of the domain.
