...
const free = (label: string): Locker =>
  unwrap(
    Locker.create({
      id: `locker-${label}`,
...
/**
 * A locker repository whose release fails after the parcel has been read.
 *
 * The one corruption worth a dedicated fixture: a parcel marked collected while
 * its locker stays occupied leaves the locker dead forever and the parcel gone
 * from the system.
 */
...
const setup = (
  ...
  const held = options.lockers ?? [free("S1"), free("S2")]
  const lockers = (
...
const storeOne = async (store: StorePackageService) =>
  unwrap(
    await store.execute({
      stationId: STATION_ID,
...
/** No acting user: nobody signed in to collect, which is the point of the flow. */
const collection = (pickupCode: string) => ({ pickupCode, audit: PUBLIC })

describe("collecting a package", () => {
  it("frees the locker, marks the parcel collected and returns the fee", async () => {
    const { store, retrieve, lockers, stored, clock } = setup()

    const { pickupCode } = await storeOne(store)
    clock.advanceBy("2d")

    const result = await retrieve.execute(collection(pickupCode))
    ...
    expect(result.value.chargeableDays).toBe(2)
    ...
    expect((await lockers.findById("locker-S1"))?.isAvailable()).toBe(true)
  })

  it("finds the parcel from the code alone, with nothing else to go on", async () => {
  ...
  it("leaves the locker usable for the next delivery straight away", async () => {
    const { store, retrieve } = setup({ lockers: [free("S1")] })

    const first = await storeOne(store)
    await retrieve.execute(collection(first.pickupCode))
    ...
    // succeeding proves the first was really given back.
    const second = await storeOne(store)

    expect(second.lockerLabel).toBe("S1")
  })

  it("charges the fee the parcel was priced at, not just the one it reported", async () => {
    ...
    // Persisted, not only returned: the fee is what the operator invoiced, and a
  ...
  describe("rejected collections", () => {
    it("refuses a code no parcel is waiting under", async () => {
      const { store, retrieve } = setup()
    ...
    it.each(["abc", "12345", "1234567", "", "40291 "])(
      ...
      async (code) => {
        ...
        await storeOne(store)
        const result = await retrieve.execute(collection(code))

        // Malformed rather than invalid: the shape is wrong, which is a
        // correctable mistake and says nothing about which codes are live.
    ...
    it("answers a replayed code exactly as it answers an unknown one", async () => {
      ...
      // parcel had gone.
      expect(JSON.stringify(replayed)).toEqual(JSON.stringify(unknown))
    })
  ...
  it("does not mark a parcel collected when its locker cannot be freed", async () => {
    const { store, retrieve, stored, lockers } = setup({
      lockers: [free("S1")],
    ...
    await expect(retrieve.execute(collection(pickupCode))).rejects.toThrow(
      "the locker row could not be written"
    )

    // The write that fails takes the whole collection with it. A parcel recorded
    // as collected behind an occupied locker is the worst state this system can
    // reach: the locker is dead and the parcel is gone.
    expect(stored[0].status).toBe("stored")
    expect((await lockers.findById("locker-S1"))?.isAvailable()).toBe(false)
  })
})
